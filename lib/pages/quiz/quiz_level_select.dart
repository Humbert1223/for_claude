...
class QuizLevelSelect extends StatefulWidget {
  ...
  @override
  QuizLevelSelectState createState() =>
      QuizLevelSelectState(); // Simplified syntax
}

class QuizLevelSelectState extends State<QuizLevelSelect> {
  ...
  Future<void> _fetchLevels() async {
    try {
      final fetchedLevels =
          await MasterCrudModel('level').search(paginate: '0');
      setState(() {
        levels = List<Map<String, dynamic>>.from(fetchedLevels);
      });
    } finally {
      setState(() {
        isLoading = false;
